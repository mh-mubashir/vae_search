...
def main():
    ...
    csv_path = os.path.join(project_root, "graph_generation", "vaegan", "vaegan.csv")
    figures_dir = os.path.join(project_root, "graph_generation", "vaegan", "figures")
    output_path = os.path.join(figures_dir, "vaegan_recon_gen_grid.png")

    # Read CSV and collect entries (orig, recon, gen)
    originals = []
    recons = []
    gens = []
    ...
    if len(originals) == 0:
        raise RuntimeError("No rows read from vaegan.csv – check the CSV format.")
    ...
    # Add left-side labels: "Original", "Reconstruction", "Generated"
    ...
    for row_idx, text in enumerate(labels):
        # Compute text bounding box to determine width/height
        try:
    ...
    print(f"Saved VAEGAN recon/gen grid with labels to: {output_path}")
