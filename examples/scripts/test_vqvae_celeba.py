#!/usr/bin/env python3
"""
Script to test pretrained VQ-VAE model on CelebA dataset.

This script:
1. Loads a pretrained VQ-VAE model from HuggingFace Hub or local path
...
Usage:
    # Test a locally trained model (no pretrained model available on HuggingFace Hub)
    ...
    python training.py --dataset celeba --model_name vqvae --model_config configs/celeba/vqvae_config.json --training_config configs/celeba/vqvae_training_config_cpu.json --nn convnet
"""
...
from pythae.models import AutoModel, VQVAE, VQVAEConfig
...
def load_celeba_data(data_path: str, num_samples: int = None):
    ...
    if not os.path.exists(train_path) or not os.path.exists(eval_path):
        raise FileNotFoundError(
            ...
            "You can download CelebA using: python data-download-fast.py celeba -o data"
        )
...
def load_model(model_path: str, device: str = "cuda"):
    """Load VQ-VAE model from HuggingFace Hub or local path.
    ...
    if os.path.exists(model_path):
        ...
        try:
            ...
            logger.error(
                ...
                "python training.py --dataset celeba --model_name vqvae"
            )
            raise
...
def test_generation(model, train_data, device, num_samples=25, output_dir=None):
    ...
    # 3. MAF Sampler
    logger.info("Testing MAFSampler...")
    try:
        ...
        maf_sampler = MAFSampler(model=model, sampler_config=maf_config)
        # Fit the sampler
        train_tensor = torch.tensor(train_data[:1000], dtype=torch.float32).to(device)
        maf_sampler.fit(train_tensor)
        # Generate samples
        with torch.no_grad():
            gen_maf = maf_sampler.sample(num_samples=num_samples, return_gen=True)
        results["maf"] = gen_maf.cpu().numpy()
        logger.info("MAFSampler completed!")
    except Exception as e:
        logger.warning(f"MAFSampler failed: {e}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Test pretrained VQ-VAE model on CelebA dataset"
    )
    ...
    parser.add_argument(
        ...
        "or local path to model directory. Note: No pretrained VQ-VAE model is currently available "
        "on HuggingFace Hub, so you'll need to train a model first or provide a local path.",
    )
    ...
    logger.info("=" * 80)
    logger.info("VQ-VAE CelebA Testing Script")
    logger.info("=" * 80)
